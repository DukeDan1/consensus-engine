...
|---|---|---|
| `OPENAI_RESPONSES_MODEL` | `gpt-5.5` | Default text/responses model for OpenAI. Used across all services (moderation, fact-checking, analysis, ontology, simulation scripts). |
| `OPENAI_MODERATION_MODEL` | Value of `OPENAI_RESPONSES_MODEL` → `gpt-5.5` | Model used specifically for AI content moderation. Falls back to the general OpenAI model. |
| `OPENAI_EMBED_MODEL` | — | Model used for generating text embeddings (ontology classification). |
| `OPENAI_IMAGE_MODEL` | `gpt-image-2` | Model used for OpenAI image generation. |
| `GROK_RESPONSES_MODEL` | `grok-4.3` | Default text model for Grok responses. |
| `GROK_IMAGE_MODEL` | — | Model used for Grok image generation. |
| `OPENROUTER_RESPONSES_MODEL` | `openai/gpt-5.5` | Default text model for OpenRouter responses. |
...
# GROK_RESPONSES_MODEL=grok-4.3
# OPENROUTER_RESPONSES_MODEL=openai/gpt-5.5

# ── Force a specific provider (optional) ──
# FORCED_AI_PROVIDER=openai
# FORCED_AI_MODEL=gpt-5.5
